...
for delay in (50,):
    ...
    for rate in (50,):
        ...
        rep_counter = 0
        while True:
            subprocess.run("ps aux | grep -v grep | grep fq_detection_server | awk '{print $2}' | xargs kill", shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            subprocess.run("ps aux | grep -v grep | grep fq_detection_client | awk '{print $2}' | xargs kill", shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            ...
            server_popen = h2.popen(f'../fq_detection_server'.split(' '), **debug)
            if args.iperf:
            ...
            client_popen = h1.popen(f'../fq_detection_client -s 192.168.0.2'.split(' '), **{"stdout": None, "stderr": None})
